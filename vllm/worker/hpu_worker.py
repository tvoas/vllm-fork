...
class HPUWorker(LocalOrDistributedWorkerBase):
    ...
    def initialize_cache(self, num_gpu_blocks: int,
        ...
        num_block_splits = 1 if envs.VLLM_USE_SHARED_BLOCK_MANGERS \
            else self.parallel_config.pipeline_parallel_size
        raise_if_cache_size_invalid(
            num_gpu_blocks, self.cache_config.block_size,
            self.model_config.max_model_len,
            self.parallel_config.pipeline_parallel_size)
        target_gpu_blocks = int(num_gpu_blocks // num_block_splits)
        target_cpu_blocks = int(num_cpu_blocks // num_block_splits)
    ...
    def _init_cache_engine(self):
        ...
        if envs.VLLM_USE_SHARED_BLOCK_MANGERS:
            singleton_cache = HPUCacheEngine(self.cache_config,
                                             self.model_config,
