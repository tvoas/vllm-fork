...
class LocalOrDistributedWorkerBase(WorkerBase):
    ...
    def execute_model(
        ...
        torch.hpu.synchronize()
        if f"02.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.execute_worker" not in self.data_logs:
            self.data_logs[f"02.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.execute_worker"] = []
        self.data_logs[f"02.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.execute_worker"] += [time.perf_counter() - tim1]
        ...
        if not get_pp_group().is_first_rank:
            ...
            if f"03.1.LocalOrDistributedWorkerBase.1.execute_model.{'prompt' if is_prompt else 'decode'}.recv_tensor_dict.hang_time" not in self.data_logs:
                self.data_logs[f"03.1.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.recv_tensor_dict.hang_time"] = []
            self.data_logs[f"03.1.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.recv_tensor_dict.hang_time"] += [get_pp_group().hang_time]
            if f"03.2.LocalOrDistributedWorkerBase.1.execute_model.{'prompt' if is_prompt else 'decode'}.recv_tensor_dict.network_time" not in self.data_logs:
                self.data_logs[f"03.2.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.recv_tensor_dict.network_time"] = []
            self.data_logs[f"03.2.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.recv_tensor_dict.network_time"] += [get_pp_group().network_time]
        ...
        if model_input.is_first_multi_step and model_input.is_last_step:
            title = f"19.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.execute_model.single_step"
        elif model_input.is_first_multi_step:
            title = f"19.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.execute_model.first_multi_step"
        elif model_input.is_last_step:
            title = f"19.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.execute_model.last_multi_step"
        else:
            title = f"19.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.execute_model.mid_multi_step"
        ...
        if not get_pp_group().is_last_rank:
            ...
            get_pp_group().send_tensor_dict(output.tensors,
                                            all_gather_group=get_tp_group())
            torch.hpu.synchronize()
            if f"20.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.send_tensor_dict" not in self.data_logs:
                self.data_logs[f"20.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.send_tensor_dict"] = []
            self.data_logs[f"20.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}.send_tensor_dict"] += [time.perf_counter() - tim1]
            if f"21.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}" not in self.data_logs:
                self.data_logs[f"21.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}"] = []
            self.data_logs[f"21.LocalOrDistributedWorkerBase.execute_model.{'prompt' if is_prompt else 'decode'}"] += [time.perf_counter() - start_time]
            return [None]
        if (self.observability_config is not None
                and self.observability_config.collect_model_execute_time
