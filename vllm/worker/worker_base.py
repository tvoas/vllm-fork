...
class WorkerBase:
    ...
    def __init__(
        ...
        self.worker_lock = threading.Lock if self.current_platform.is_hpu(
        ) else nullcontext()
...
class LocalOrDistributedWorkerBase(WorkerBase):
    ...
    def execute_model(
        ...
        collect_model_execute_time = self.observability_config is not None \
        ...
        with self.worker_lock:
            ...
            if not get_pp_group().is_first_rank:
                ...
                assert not (defer and collect_model_execute_time), (
                    'PP deferred receive does not support '
                    'observe model execute time')
