...
class HPUModelRunner(HPUModelRunnerBase[ModelInputForHPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        logfn(f"HPUModelRunner.execute_model.info_1: in old branch")
    ...
    def execute_model_multi(
        ...
        logfn(f"HPUModelRunner.execute_model_multi.info_01: in new branch")
        is_prompt = model_input.is_prompt
        is_multi_step = not (model_input.is_first_multi_step and model_input.is_last_step) or num_steps > 1
        self.has_patched_prev_output = False
        use_delayed_sampling = self.use_delayed_sampling and not warmup_mode
        ...
        assert model_input.input_tokens is not None
        output = None
        logfn(f"HPUModelRunner.execute_model_multi.info_02")
        if use_delayed_sampling and not model_input.is_prompt and \
                self.is_driver_worker:
            num_cached = len(self.cached_step_outputs)
            assert num_cached > 0
            ...
            for i in range(num_cached):
                ...
                model_input.input_tokens.index_copy_(
                    0, target_indices, self.cached_step_outputs[i])
                htorch.core.mark_step()
        logfn(f"HPUModelRunner.execute_model_multi.info_03")
        if self.lora_config:
            assert model_input.lora_requests is not None
            assert model_input.lora_mapping is not None
            self.set_active_loras(model_input.lora_requests,
                                    model_input.lora_mapping)
        logfn(f"HPUModelRunner.execute_model_multi.info_04")
        # Rank!=0 workers has is_prompt==None
        if use_delayed_sampling and not model_input.is_prompt and \
                model_input.input_tokens.size(1) == 1:
            logfn(f"HPUModelRunner.execute_model_multi.info_05")
            if self.is_driver_worker:
                model_kwargs_broadcast_data = {
                    "input_tokens": model_input.input_tokens
                }
            ...
            logfn(f"HPUModelRunner.execute_model_multi.info_06")
            input_tokens = model_input.input_tokens
        input_positions = model_input.input_positions
        attn_metadata = model_input.attn_metadata
        sampling_metadata = model_input.sampling_metadata
        ...
        batch_size = input_tokens.size(0)
        seq_len = self._seq_len(attn_metadata)
        logfn(f"HPUModelRunner.execute_model_multi.info_07")
        num_patches = self._get_num_patches_from_model_input(model_input)
        logfn(f"HPUModelRunner.execute_model_multi.info_08")
        use_graphs = self._use_graphs(batch_size=batch_size,
                                        seq_len=seq_len,
                                        is_prompt=is_prompt,
                                        num_patches=num_patches)
        self._check_config(batch_size, seq_len, attn_metadata, warmup_mode)
        logfn(f"HPUModelRunner.execute_model_multi.info_09")
        ...
        if self.lora_config:
            ...
            lora_mask, lora_logits_mask = self.create_lora_mask(
                input_tokens, model_input.lora_ids,
                attn_metadata.is_prompt)
        logfn(f"HPUModelRunner.execute_model_multi.info_10")
        if model_input.multi_modal_kwargs is not None \
            and 'embed_is_patch' in model_input.multi_modal_kwargs:
            logfn(f"HPUModelRunner.execute_model_multi.info_11")
            ...
            def fix_embed_is_patch(embed_is_patch):
                if isinstance(embed_is_patch, torch.Tensor):
                    ...
                    return None
            logfn(f"HPUModelRunner.execute_model_multi.info_12")
            model_input.multi_modal_kwargs[
                'embed_is_patch'] = fix_embed_is_patch(
                    model_input.multi_modal_kwargs['embed_is_patch'])
        logfn(f"HPUModelRunner.execute_model_multi.info_13")
        execute_model_kwargs = {
            "input_ids": input_tokens,
            "positions": input_positions,
            "kv_caches": kv_caches,
        ...
        logfn(f"HPUModelRunner.execute_model_multi.info_14")
        if previous_hidden_states is not None:
            logfn(f"HPUModelRunner.execute_model_multi.info_15")
            # HPU will pad up to block_size,
            # pad previous_hidden_states as well
            previous_hidden_states = previous_hidden_states.unsqueeze(
                1).expand(-1, input_tokens.shape[-1], -1)
            ...
            execute_model_kwargs.update(
                {"previous_hidden_states": previous_hidden_states})
        logfn(f"HPUModelRunner.execute_model_multi.info_16")
        if htorch.utils.internal.is_lazy():
            execute_model_kwargs.update(
                {"bypass_hpu_graphs": not use_graphs})

        logfn(f"HPUModelRunner.execute_model_multi.info_17")
        htorch.core.mark_step()
        logfn(f"HPUModelRunner.execute_model_multi.info_18")
        if self.is_driver_worker:
            model_event_name = ("model_"
                                f"{'prompt' if is_prompt else 'decode'}_"
                                f"bs{batch_size}_"
        ...
        if is_multi_step or use_delayed_sampling:
            ...
            self.sampler.include_gpu_probs_tensor = True
        cache_orig_output_tokens_len: List[Dict] = []
        logfn(f"HPUModelRunner.execute_model_multi.info_19")
            ...
            if len(cache_orig_output_tokens_len) > 0:
                ...
                for i in range(len(cache_orig_output_tokens_len)):
                    ...
                    for j, data in seq_group_metadata.seq_data.items():
                        ...
                        data.output_token_ids = \
                            data.output_token_ids[:orig_output_tokens_len]
        logfn(f"HPUModelRunner.execute_model_multi.info_20")
        if not model_input.is_first_multi_step and not (self.is_driver_worker and get_pp_group().is_last_rank):
            src = (self.parallel_config.pipeline_parallel_size - 1) * self.parallel_config.tensor_parallel_size
            logfn(f"HPUModelRunner.execute_model_multi.info_21")
            broadcast_data = world_broadcast_tensor_dict(src=src)
            logfn(f"HPUModelRunner.execute_model_multi.info_22")
            if 'early_exit' in broadcast_data and broadcast_data[
                    'early_exit']:
                logfn(f"HPUModelRunner.execute_model_multi.info_23")
        ...
        logfn(f"HPUModelRunner.execute_model_multi.info_24")
        profiler_args = {
            'real_seq_len': model_input.seq_lens,
            'real_batch_size': real_batch_size
        }
        ...
        if self.model_is_mrope:
            logfn(f"HPUModelRunner.execute_model_multi.info_25")
            # run multimodal encoder for mrope before forward
            inputs_embeds = \
                self.model.compute_input_embeddings_for_mrope(
                    **execute_model_kwargs
            ...
            execute_model_kwargs.pop('pixel_values', None)
            execute_model_kwargs.pop('image_grid_thw', None)
        logfn(f"HPUModelRunner.execute_model_multi.info_26")
        with self.profiler.record_event('internal',
                                        model_event_name,
                                        args=profiler_args):
            logfn(f"HPUModelRunner.execute_model_multi.info_27")
            hidden_states = self.model.forward(
                **execute_model_kwargs,
                selected_token_indices=sampling_metadata.
                selected_token_indices)
            logfn(f"HPUModelRunner.execute_model_multi.info_28")
            if warmup_mode:
                logfn(f"HPUModelRunner.execute_model_multi.info_29")
                torch.hpu.synchronize()
                import torch.distributed as dist
                logfn(f"HPUModelRunner.execute_model_multi.info_30")
                if dist.is_initialized():
                    logfn(f"HPUModelRunner.execute_model_multi.info_31")
                    get_tp_group().barrier()
                    logfn(f"HPUModelRunner.execute_model_multi.info_32")

        if self.lora_config:
            LoraMask.setLoraMask(
                lora_logits_mask.index_select(
                    0, sampling_metadata.selected_token_indices))
        logfn(f"HPUModelRunner.execute_model_multi.info_33")
        if not get_pp_group().is_last_rank:
            logfn(f"HPUModelRunner.execute_model_multi.info_34")
            return hidden_states
        logfn(f"HPUModelRunner.execute_model_multi.info_35")
        ...
        if (use_delayed_sampling and self.is_driver_worker
            ...
            # we obtain the actual sampled results in advance
            self._patch_prev_output()
        logfn(f"HPUModelRunner.execute_model_multi.info_36")
        ...
        with self.profiler.record_event(
            ...
            if not is_multi_step:
                sampling_metadata.selected_token_indices = None
            logfn(f"HPUModelRunner.execute_model_multi.info_37")
            logits = self.model.compute_logits(hidden_states,
                                                sampling_metadata)
        logfn(f"HPUModelRunner.execute_model_multi.info_38")

        htorch.core.mark_step()
        # Only perform sampling in the driver worker.
        if not self.is_driver_worker:
            logfn(f"HPUModelRunner.execute_model_multi.info_39")
            return []
        logfn(f"HPUModelRunner.execute_model_multi.info_40")

        if use_delayed_sampling:
            fake_output = self._delayed_sampler_outputs(model_input)
        elif model_input.async_callback is not None:
            model_input.async_callback()
        logfn(f"HPUModelRunner.execute_model_multi.info_41")
        ...
        with self.profiler.record_event(
                'internal', ('sample_'
                                ...
                                f'seq{seq_len}'),
                args=profiler_args):
            logfn(f"HPUModelRunner.execute_model_multi.info_42")
            output = self.sampler(
                logits=logits,
                sampling_metadata=sampling_metadata,
            )
            logfn(f"HPUModelRunner.execute_model_multi.info_43")
            if output.sampled_token_ids is None:
                output.sampled_token_ids_cpu = output[0].samples[0].output_token
            else:
                output.sampled_token_ids_cpu = output.sampled_token_ids
            ...
            if use_delayed_sampling and self.is_driver_worker:
                ...
                self.cached_step_outputs.append(output)
                self.cached_step_inputs.append(model_input)
            logfn(f"HPUModelRunner.execute_model_multi.info_44")
        htorch.core.mark_step()
        logfn(f"HPUModelRunner.execute_model_multi.info_45")
        if use_delayed_sampling \
            and model_input.async_callback is not None:
            model_input.async_callback()
        logfn(f"HPUModelRunner.execute_model_multi.info_46")
        if is_multi_step and not model_input.is_last_step:
            logfn(f"HPUModelRunner.execute_model_multi.info_47")
            if model_input.is_first_multi_step:
                logfn(f"HPUModelRunner.execute_model_multi.info_48")
                if model_input.async_callback is not None:
                    ctx = model_input.async_callback.keywords[  # type: ignore
                        "ctx"]
                    seq_group_metadata_list = \
                    ...
                    raise RuntimeError(
                        "seq_group_metadata_list is uninitialized")
                logfn(f"HPUModelRunner.execute_model_multi.info_49")
                for seq_idx, seq_group_metadata in enumerate(
                        seq_group_metadata_list):
                    # Skip empty steps
                    ...
                    for j, data in seq_group_metadata.seq_data.items():
                        cache_orig_output_tokens_len[seq_idx][j] = \
                            len(data.output_token_ids)
            logfn(f"HPUModelRunner.execute_model_multi.info_50")
            seq_group_metadata_list, _, _ = self._add_dummy_seq(
                seq_group_metadata_list, is_prompt=False)
            for seq_group_metadata in seq_group_metadata_list:
                for data in seq_group_metadata.seq_data.values():
                    ...
                    if len(data.output_token_ids) < max_output_len - 1:
                        ...
                        if not is_multi_step:
                            ...
                            try_revert_dummy_output_tokens()
                            return []
            logfn(f"HPUModelRunner.execute_model_multi.info_51")

            result = self._prepare_decode(seq_group_metadata_list,
                                            output=output)
            logfn(f"HPUModelRunner.execute_model_multi.info_52")
            ...
            if self.lora_config:
                ...
                lora_mask, lora_logits_mask = self.create_lora_mask(
                    result.input_tokens, result.lora_ids, False)
            logfn(f"HPUModelRunner.execute_model_multi.info_53")
            ...
            logfn(f"HPUModelRunner.execute_model_multi.info_54")
            model_kwargs_broadcast_data = {
                "input_ids": result.input_tokens,
                "positions": result.input_positions,
                "attn_metadata": vars(result.attn_metadata),
            ...
            logfn(f"HPUModelRunner.execute_model_multi.info_55")
            src = (self.parallel_config.pipeline_parallel_size - 1) * self.parallel_config.tensor_parallel_size
            logfn(f"HPUModelRunner.execute_model_multi.info_56")
            world_broadcast_tensor_dict(model_kwargs_broadcast_data, src=src)
            logfn(f"HPUModelRunner.execute_model_multi.info_57")
        else:
            logfn(f"HPUModelRunner.execute_model_multi.info_58")
            try_revert_dummy_output_tokens()
            logfn(f"HPUModelRunner.execute_model_multi.info_59")
        logfn(f"HPUModelRunner.execute_model_multi.info_60")
        if self.is_driver_worker and self.profiler.enabled:
            # Stop recording 'execute_model' event
            self.profiler.end()
            event_end = self.profiler.get_timestamp_us()
            counters = self.profiler_counter_helper.get_counter_dict(
                ...
                is_prompt=is_prompt)
            self.profiler.record_counter(self.event_start, counters)
        logfn(f"HPUModelRunner.execute_model_multi.info_61")
        if not is_multi_step:
            logfn(f"HPUModelRunner.execute_model_multi.info_62")
            if self.spec_decode_enabled and isinstance(
                    output, SamplerOutput):
                output.sampled_token_ids = output.sampled_token_ids[:
                                                                    real_batch_size]
            ...
            logfn(f"HPUModelRunner.execute_model_multi.info_63")
            output = self._decode_sampler_outputs(
                model_input) if self.is_driver_worker else []
            torch.hpu.synchronize()
            return output if type(output) is list else [output]
