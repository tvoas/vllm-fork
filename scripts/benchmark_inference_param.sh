...
MNBT=${15:-${MAX_MODEL_LEN}}
GMU=${16:-0.75}
RESULTS_DIR=${17:-logs/test-results}

if [ "$DO_PROFILE" == "true" ]; then
  export VLLM_PROFILER_ENABLED=True
  export VLLM_TORCH_PROFILER_DIR=$( realpath ./vllm_profile )
  ...
  #export VLLM_ENGINE_PROFILER_ENABLED=true
  #export VLLM_ENGINE_PROFILER_WARMUP_STEPS=30
  #export VLLM_ENGINE_PROFILER_STEPS=20
  #export VLLM_ENGINE_PROFILER_REPEAT=3
...
export VLLM_GPU_MEMORY_UTILIZATION=${VLLM_GPU_MEMORY_UTILIZATION:-${GMU}}
export VLLM_GRAPH_RESERVED_MEM=${VLLM_GRAPH_RESERVED_MEM:-0.02}
...
INPUT_LEN=8196
OUTPUT_LEN=2048
NUM_PROMPTS=$(( 3 * PP_SIZE * MAX_NUM_SEQS  ))

numactl -C 56-111 -m 1 \
